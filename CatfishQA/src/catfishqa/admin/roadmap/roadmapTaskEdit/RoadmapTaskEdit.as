package catfishqa.admin.roadmap.roadmapTaskEdit 
{
	...
	public class RoadmapTaskEdit extends NativeWindowInitOptions 
	{
		...
		public function RoadmapTaskEdit(projectSelect:String, sprintSelectID:String, data:Array) 
		{
			super();
			...
			_newWindow.title = "Редактировать задачу"; 
		...
		private function SelectRelise():void
		{
			var date:String = _data[0].Релиз;
		...
		{
			var n:int = _version.length;
			for (var m:int = 0; m < n; m++)
			{
				if (_version[m].data == _data[0].Версия)
				{
					_comboBox4.selectedIndex = m;
					break;
				}
			}
		}
		
		private function SelectDevBegin():void
		{
			var date:String = _data[0].DEV_Begin;
			var year:String = date.charAt(0) + date.charAt(1) + date.charAt(2) + date.charAt(3);
			var mount:String = date.charAt(5) + date.charAt(6);
			var day:String = date.charAt(8) + date.charAt(9);
			...
			for (var d:int = 0; d < n; d++)
			{
				if (_days[d].data == day)
				{
					_comboBox5.selectedIndex = d;
					break;
				}
			}
		}
		
		private function SelectDevEnd():void
		{
			var date:String = _data[0].DEV_End;
			var year:String = date.charAt(0) + date.charAt(1) + date.charAt(2) + date.charAt(3);
			var mount:String = date.charAt(5) + date.charAt(6);
			var day:String = date.charAt(8) + date.charAt(9);
			...
			for (var d:int = 0; d < n; d++)
			{
				if (_days[d].data == day)
				{
					_comboBox7.selectedIndex = d;
					break;
				}
			}
		}
		
		private function SelectQaBegin():void
		{
			var date:String = _data[0].QA_Begin;
			var year:String = date.charAt(0) + date.charAt(1) + date.charAt(2) + date.charAt(3);
			var mount:String = date.charAt(5) + date.charAt(6);
			var day:String = date.charAt(8) + date.charAt(9);
			...
			for (var d:int = 0; d < n; d++)
			{
				if (_days[d].data == day)
				{
					_comboBox9.selectedIndex = d;
					break;
				}
			}
		}
		
		private function SelectQaEnd():void
		{
			var date:String = _data[0].QA_End;
			var year:String = date.charAt(0) + date.charAt(1) + date.charAt(2) + date.charAt(3);
			var mount:String = date.charAt(5) + date.charAt(6);
			var day:String = date.charAt(8) + date.charAt(9);
		...
		private function onButton1MouseClick(e:MouseEvent):void 
		{
			var sqlCommand:String = "UPDATE roadmap_tasks SET "
								+ "roadmap_tasks_release = '" + _textBox2.text + "-" + _comboBox3.selectedItem.data + "-" + _comboBox2.selectedItem.data + "', "
								+ "roadmap_tasks_version = '" + _comboBox4.selectedItem.data + "', "
								+ "roadmap_tasks_name = '" + _textBox3.text + "', "
								+ "roadmap_tasks_link = '" + _textBox4.text + "', "
								+ "roadmap_tasks_dev_begin = '" + _textBox5.text + "-" + _comboBox6.selectedItem.data + "-" + _comboBox5.selectedItem.data + "', "
								+ "roadmap_tasks_dev_end = '" + _textBox6.text + "-" + _comboBox8.selectedItem.data + "-" + _comboBox7.selectedItem.data + "', "
								+ "roadmap_tasks_qa_begin = '" + _textBox7.text + "-" + _comboBox10.selectedItem.data + "-" + _comboBox9.selectedItem.data + "', "
								+ "roadmap_tasks_qa_end = '" + _textBox8.text + "-" + _comboBox12.selectedItem.data + "-" + _comboBox11.selectedItem.data + "', "
								+ "roadmap_tasks_sprint_id = '" + _comboBox1.selectedItem.ID + "' "
								+ "WHERE roadmap_tasks_id = " + _data[0].ID;
